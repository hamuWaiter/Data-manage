<template>
  <div>
    ...
    <el-table :data="items">
      ...
      <el-table-column prop="name" label="装备名称" width="120"></el-table-column>
      <el-table-column prop="icon" label="图标">
        <template slot-scope="scope">
          <!-- scope.row表示整个一行的数据 -->
          <img :src="scope.row.icon" alt="" style="height:3.5em;">
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$http.get("rest/goods"); //请求页面数据
